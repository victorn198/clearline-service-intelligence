...
def build(source:Path)->None:
    ...
    con.execute("""CREATE OR REPLACE TABLE topic_signal AS SELECT
      CASE
        WHEN lower(coalesce(narrative,issue)) SIMILAR TO '%(identity|fraud|not mine|unauthorized)%' THEN 'Identity & fraud'
        WHEN lower(coalesce(narrative,issue)) SIMILAR TO '%(credit report|incorrect information|score)%' THEN 'Credit reporting accuracy'
        WHEN lower(coalesce(narrative,issue)) SIMILAR TO '%(payment|fee|interest|charge)%' THEN 'Payments & fees'
        WHEN lower(coalesce(narrative,issue)) SIMILAR TO '%(collection|collector|debt)%' THEN 'Debt collection'
        WHEN lower(coalesce(narrative,issue)) SIMILAR TO '%(close|cancel|access|blocked)%' THEN 'Account access'
    ...
    con.execute(f"""COPY (SELECT received_at::DATE received_date,product,company,state,
