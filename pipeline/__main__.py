...
def build(source:Path)->None:
    ...
    # Static payloads define labels and initial metadata only. Runtime comparisons
    # are calculated from non-overlapping date windows in DuckDB-WASM.
    metric=lambda ident,en,pt,val,prev,fmt='integer',improvement='up':{'id':ident,'label':loc(en,pt),'value':val,'previous':0,'format':fmt,'improvement':improvement}
    ...
    context_metrics=[metric('complaints','Complaints in scope','Reclamações no recorte',total,previous,'integer','neutral'),metric('states','Jurisdictions represented','Jurisdições representadas',state_count,state_count,'integer','neutral'),metric('products','Products','Produtos',product_count,product_count,'integer','neutral'),core[2]]
    specs=[
