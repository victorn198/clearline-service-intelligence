...
export function BarChart({ data, lang, onSelect, direction = 'neutral' }: { data: BreakdownRow[]; lang: Lang; onSelect?: (name: string) => void; direction?: Direction }) {
  ...
  const priorSeries={name:lang==='pt'?'Período anterior':'Prior period',type:'bar',data:seriesData.map(row=>row.hasPrevious?row.previous:null),barWidth:8,itemStyle:{color:'#596780',borderRadius:[0,5,5,0]},emphasis:{disabled:true}}
  const currentSeries={name:lang==='pt'?'Período atual':'Current period',type:'bar',data:seriesData,barWidth:10,itemStyle:{color:'#3978ff',borderRadius:[0,5,5,0]},emphasis:{disabled:true},z:2,label:{show:true,position:'right',formatter:label,rich:{value:{color:'#d8dfef'},divider:{color:'#596780'},good:{color:'#48d69f',fontWeight:700},bad:{color:'#ff617d',fontWeight:700},info:{color:'#6da0ff',fontWeight:700},new:{color:'#6da0ff',fontWeight:700},neutral:{color:'#91a0bc'}}}}
  const tooltip=(params:Array<{dataIndex:number}>)=>{const index=params[0]?.dataIndex??0,row=seriesData[index],name=sorted[index]?.name??'',color=tone(row.delta)==='good'?'#48d69f':tone(row.delta)==='bad'?'#ff617d':'#6da0ff';return `<b>${name}</b><br/>${lang==='pt'?'Atual':'Current'}: ${display(row.value)}<br/>${lang==='pt'?'Anterior':'Prior'}: ${row.hasPrevious?display(row.previous):'—'}<br/>${lang==='pt'?'Variação':'Change'}: <span style="color:${color};font-weight:700">${comparison({data:row})}</span>`}
  const option = { animationDuration: 450, grid: { left: 12, right: 112, top: 12, bottom: 12, containLabel: true }, tooltip: { trigger: 'axis',axisPointer:{type:'shadow'},backgroundColor:'#171f32',borderColor:'#2b3853',textStyle:{color:'#f7f9ff'},formatter:tooltip }, xAxis: { type: 'value', splitNumber: 3, max: mode === 'share' ? 100 : undefined, splitLine: { lineStyle: { color: '#202a3e' } }, axisLabel: { color: '#91a0bc', fontSize:10, hideOverlap:true, formatter: display } }, yAxis: { type: 'category', data: sorted.map(d => d.name), axisLine: { show: false }, axisTick: { show: false }, axisLabel: { color: '#d8dfef', width: 120, overflow: 'truncate' } }, series:[priorSeries,currentSeries] }
  const events = onSelect ? { click: (params: { name: string }) => onSelect(params.name) } : undefined
  return <div className="chart-block"><div className="chart-modes" role="group" aria-label={lang === 'pt' ? 'Modo de comparação' : 'Comparison mode'}><small className="comparison-key"><span><i className="current"/> {lang==='pt'?'Atual':'Current'}</span><span><i className="prior"/> {lang==='pt'?'Anterior':'Prior'}</span></small><button className={mode === 'value' ? 'active' : ''} onClick={() => setMode('value')}>{lang === 'pt' ? 'Valor' : 'Value'}</button><button className={mode === 'share' ? 'active' : ''} onClick={() => setMode('share')}>{lang === 'pt' ? 'Participação' : 'Share'}</button></div><div className="chart-viewport"><ReactEChartsCore echarts={echarts} option={option} onEvents={events} notMerge={true} style={{height:300,width:'100%'}} /></div></div>
}
